        ...
        <div id="planet" style="background-image:url({{ asset('img/headers/overview/' . $header_filename) }}.jpg);">
            ...
            <div id="buffBar" class="sliderWrapper">
                <div data-uuid="" data-id="" class="add_item">
                    <a id="js_openItemSlider" class="activate_item border3px tooltip" href="javascript:void(0);" ref="1" title="{{ __('t_shop_items.overlay_title') }}"></a>
                </div>

                {{-- Use <div> instead of <ul>/<li> so external anythingSlider script
                     does NOT auto-inject inline width/height (would stretch tiles to 538px). --}}
                @php $active_boosts = $active_boosts ?? []; @endphp
                <div class="active_items_wrap @if (empty($active_boosts)) hidden @endif">
                    @foreach ($active_boosts as $boost)
                        @php
                            // Tooltip body — Tipped reads `title` as "TITLE|BODY".
                            // Body uses <br /> for line breaks (preserved by sanitizeTooltip).
                            $tipBody = '<span style="color:#9c0;font-weight:bold;">' . __('t_shop_items.label_time_left') . ': <span class="js_boostRemaining" data-expires-ts="' . $boost['expires_at_ts'] . '">—</span></span><br /><br />'
                                . '<span style="color:#9c0;font-weight:bold;">' . __('t_shop_items.label_phase_in_use') . '</span><br /><br />'
                                . e($boost['description']) . '<br /><br />'
                                . __('t_shop_items.label_duration') . ': ' . e($boost['duration_label']) . '<br /><br />'
                                . __('t_shop_items.label_price') . ': ' . e($boost['price_label']) . ' ' . __('t_shop_items.label_dark_matter') . '<br />'
                                . __('t_shop_items.label_inventory') . ': ' . $boost['inventory_count'];
                            $tipRaw = e($boost['label']) . '|' . $tipBody;
                            $tip = str_replace(['"', "\n", "\r"], ['&quot;', '', ''], $tipRaw);
                        @endphp
                        <div class="active_boost r_{{ $boost['rarity'] }} tooltipHTML js_boostTile"
                             data-expires-ts="{{ $boost['expires_at_ts'] }}"
                             data-name="{{ $boost['label'] }}"
                             data-description="{{ $boost['description'] }}"
                             data-percent="{{ $boost['percent'] }}"
                             data-price-label="{{ $boost['price_label'] }}"
                             data-duration-label="{{ $boost['duration_label'] }}"
                             data-inventory-count="{{ $boost['inventory_count'] }}"
                             data-rarity="{{ $boost['rarity'] }}"
                             data-image-url="{{ $boost['image_url'] }}"
                             title="{!! $tip !!}">
                            <span class="item_img" style="background-image:url({{ $boost['image_url'] }});"></span>
                        </div>
                    @endforeach
                </div>
            </div>
...
" href="#TODOpage=ingame&amp;component=lfresearch">
    ...
    <style>
        ...
        #invOverlayRoot #activeBuffDetails{position:relative;width:654px;height:258px;background:#0d1014 url(/img/shop/buff-bg.png) 0 0 no-repeat;color:#f1f1f1;z-index:11;overflow:hidden;}
        ...
        #invOverlayRoot .active_item_slider .item_img .item_img_box{position:relative;width:75px;height:75px;margin:0 0 2px;background:url(/img/shop/item-frame.png) -150px 0 no-repeat;z-index:10;}
        ...
        #invOverlayRoot .active_item_slider .item_img:hover a.detail_button{background:url(/img/shop/detail-btn.png) no-repeat;}
        ...
        #invOverlayRoot #activationButton{display:table;position:relative;width:143px;height:54px;float:right;margin:0;padding:0;background:url(/img/shop/help-icon.png) 0 -108px no-repeat;color:#848484;font:700 12px/12px Verdana,Arial,sans-serif;text-align:center;text-decoration:none;cursor:pointer;}
    ...
    <div id="invOverlayRoot" aria-hidden="true">
        <div id="invOverlayDetail" style="display:block;">
            <div id="activeBuffDetails">
                ...
                @php $item_catalog = $item_catalog ?? ['pages' => []]; @endphp
                ...
                <div class="active_item_details border5px">
                    ...
                    <a id="activationButton" class="build-it_disabled activateItem" href="javascript:void(0);"><span>{{ __('t_shop_items.btn_activate') }}</span></a>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        (function () {
            ...
            var activateToken = @json($activateToken ?? csrf_token());
            ...
            function selectTile(tile) {
                ...
                if (hasActiveBoost) {
                    ...
                    if (btn) {
                        btn.className = 'build-it activateItem';
                    ...
                    if (btn) {
                        ...
                        btn.className = (can ? 'build-it' : 'build-it_disabled') + ' activateItem';
