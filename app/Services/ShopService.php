...
class ShopService
{
    ...
    public function itemToArray(ShopItem $it): array
    {
        ...
        // duration_label is a locale-neutral OGame-native format ("1s", "4s 2g", "12s 6g")
        // so we always trust the DB value over any translation override (which can be stale).
        $tDuration = $it->duration_label ?: ($hasTx && isset($tx['duration_label']) ? $tx['duration_label'] : '');
        ...
        // Disambiguate items that share the same name but have different durations
        // (e.g. "Amplificatore Argento" 7d / 30d / 90d are three separate shop rows).
        // Append the duration_label to the displayed name when the boost lasts longer
        // than a day, so users can tell variants apart at a glance.
        $tNameDisplay = $tName;
        if ((int) $it->duration_seconds > 86400 && !empty($tDuration)) {
            $tNameDisplay = $tName . ' (' . $tDuration . ')';
        }
